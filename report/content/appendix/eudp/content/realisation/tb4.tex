...
\subparagraph{Writing a value}\textbf{ }\\
...
\begin{lstlisting}[language=c]
#define EXT_HW_BASE_ADDR	0x82000000
#define HW_LED_REG    (*(volatile unsigned short *)(EXT_HW_BASE_ADDR + 192))

...
	// Writing to the FPGA's diode register
	HW_LED_REG = Sval;
\end{lstlisting}

\paragraph{Initializing the UART}\textbf{ }\\
Initialization of UART2 used to communicate with the PLC module. 
\begin{lstlisting}[language=c]
...
	// Calling the init UART2 and set Baud rate 9600. 
	initUart(UART2, B9600(Fpclk), UART_8N1);
...
...
\subparagraph{UART main function}\textbf{ }\\
...
% PHP Install
		...
		\begin{figure}[H]
			\begin{centering}
				...
				\includegraphics[width=1.0\textwidth]{content/appendix/eudp/images/php_info.png}
...
% PhpMyAdmin Install
		...
		\begin{figure}[H]
			\begin{centering}
				...
				\includegraphics[width=1.0\textwidth]{content/appendix/eudp/images/phpmyadmin.png}
