<?php
	...
	$row = $res->fetch_row();
	if($row[0]>0){
		$c_pw_pro = number_format(($row[0]*$voltage),0)."W ";
		$c_pw_con = "0W ";
	} else if ($row[0]<0){
		$c_pw_con = number_format(($row[0]*$voltage*-1),0)."W ";
		$c_pw_pro = "0W ";
	} else {
		$c_pw_pro = "0W ";
		$c_pw_con = "0W ";
	...
	$res = $con->query($sql);
	$row = $res->fetch_row();
	$tt_pw_pro = number_format(($row[0]*$voltage),0)."W ";
	...
	$res = $con->query($sql);
	$row = $res->fetch_row();
	$avg_pw_pro = number_format(($row[0]*30),0)."W ";
